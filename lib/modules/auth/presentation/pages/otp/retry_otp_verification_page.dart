...
class RetryOtpVerificationPage extends FormGenericBlocPage<OtpVerificationBloc, bool, String> {
  RetryOtpVerificationPage({super.key})
  ...
  Future<void> onSubmitSuccess(BuildContext context, FormBaseState<bool> state, {String? message}) async {
    ...
    context.read<GetProfileBloc>().add(
      SubmitEvent(sl<GetProfileUseCase>(), params: ParamsGetProfile()),
    );
  }
  ...
  Widget builder(BuildContext context, FormBaseState<bool> state) {
    return AppForm(
      ...
      labelSubmitButton: AppStrings.verifyAccount,
      onSubmit: (data) {
